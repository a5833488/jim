...
public class LoginServiceProcessor extends AbstractProtocolCmdProcessor implements LoginCmdProcessor {
	...
	public User getUser(LoginReqBody loginReqBody, ImChannelContext imChannelContext) {
		...
		User user = extengetUser(token,loginReqBody.getUserId());
	 ...
	 * @author: WChao
	 */
	public User extengetUser(String token,String usrId) {
		//demo中用map，生产环境需要用cache
//		Jedis jedis = new Jedis("192.168.0.163",6379);
//		String ss = jedis.get("60c09688-4031-42ec-bffd-874964cd8a81");
		...
		User.Builder builder = User.newBuilder()
				...
				.addGroup(Group.newBuilder().groupId("100").name("别的东西")
						.build())
		;
		//模拟的群组,正式根据业务去查数据库或者缓存;
		List<Group> groups = new ArrayList<>();
		if (usrId.equals("123")){
			groups.add(Group.newBuilder().groupId("100").name("别的东西")
					.build());

		}else if (usrId.equals("234")){
			groups.add(Group.newBuilder().groupId("100").name("新的组")
					.build());
		}else {
			groups.add(Group.newBuilder().groupId("100").name("新的组")
					.build());
	...
	public void initFriends(User.Builder builder) {
		Group myFriend = Group.newBuilder().groupId("100").name("sss的东西").build();
